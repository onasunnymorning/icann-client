...
func (c *Client) GetStateResponse() (*StateResponse, error) {
	cfg := c.Config()
	...
	if resp.StatusCode != http.StatusOK {
		// Include the URL to aid debugging of 404s and routing issues.
		return nil, fmt.Errorf("failed to get status: %d %s %s", resp.StatusCode, req.Method, req.URL.String())
	}
