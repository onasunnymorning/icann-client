...
- **Breaking:** `--auth tlsa` is now `--auth cert`, and the credentials-file `auth_type` value is now `cert` instead of `tlsa` (`client.AUTH_TYPE_TLSA` is renamed `client.AUTH_TYPE_CERT` for library callers too). "tlsa" was an internal label that never appeared on the wire; "cert" says what the flag actually selects — a TLS client certificate.
- **Breaking:** the `icann get metrica ...` command group is renamed `icann get abuse ...` (`latest`/`date`/`lists` unchanged). METRICA (formerly DAAR) is ICANN's own name for its domain-abuse reporting system; the command is now named for what it reports. The underlying library (`mosapi.Client.GetMetricaLatest` etc.) is unchanged.
- `icann get escrow status --date` now defaults to today instead of erroring when omitted, since "did today's deposit land?" is the obvious default question.
- `icann get monthly status --type` is now marked required through Cobra (shows as required in `--help`), in addition to the existing error naming the valid values.
- `rri.ResultHint` now covers every result code instead of 8 of ~21; a rejection the CLI shows always includes a plain-English hint, not just ICANN's own message.
- Error messages for an invalid `--auth`, `--env`, `--role` or `--api-version` value now read as a plain comma-separated list (e.g. `only "basic", "cert" are supported`) instead of printing a Go slice literal (`[basic cert]`).
- A missing credentials file now names the resolved path and how to fix it (`--credentials-file`, `ICANN_SHARED_CREDENTIALS_FILE`, or `icann config show`) instead of a raw `open ...: no such file or directory` error.
- An invalid `--date`/`--month` now names the expected format only, instead of leaking Go's reference-time parsing error.
...
### Added
- `icann status` — a one-shot operational summary combining SLA monitoring (`icann get tld status`) and ICANN's reporting-obligation status (`icann get reporting status`) into a single "is this TLD compliant right now?" check. Human-readable by default; `--json` for the structured data. Exits non-zero if SLA is down, any obligation is unsatisfactory, or either check fails — usable as a monitoring check.
...
- Every command now has runnable `--help` examples, and the root command documents what the CLI reports and to whom.

### Fixed
- Group commands (`get`, `submit`, `config`, `tld`, `abuse`, `escrow`, `monthly`, `reporting`) now actually reject an unknown subcommand (e.g. `icann get foo`) instead of silently printing help and exiting 0. A previous entry in this file claimed this was already fixed; it wasn't — these commands had no `RunE`, so Cobra never reached the `Args: cobra.NoArgs` check that was supposed to catch it. The root command has the same fix for an unknown top-level command.
